...
> ⚠️ **אזהרה — אל תיישם את מוסכמת ה-CamelCase של `source_entity_type` למטה.**
> הכתיב `Task` / `ChatMessage` שמתועד כאן הוא **המקור ההיסטורי לבאג**: מיגרציה
> 067 ניקתה עם `where source_entity_type = 'task'` (case-sensitive), פסחה על כל
> שורות `'Task'`, והשאירה 140 התראות יתומות בפרודקשן (נוקו ב-068). מאז:
> המפיקים יושרו (`09391c0`) והנתונים נורמלו (מיגרציה 070). **הכתיב התקף היום
> הוא `lower_snake_case` בלבד** — `task`, `issue`, `chat_message`,
> `conversation`, `calendar_event`. המסמך הזה נשאר כארכיון היסטורי של Base44.
