...
1. **`ALMOG_BILLING_BRIEF.md`** — קרא ראשון בכל סשן, לפני כל תכנון או שינוי קוד.
   קובץ מקומי בשורש, **מחוץ ל-git במכוון** (הריפו ציבורי, הקובץ מכיל פרטי תשתית).
   קובע לגבי **מצב נוכחי**: סטאק, תשתית, מודולים פעילים, משימות פתוחות, כללי עבודה.
2. **`PROJECT_CONTEXT.md`** — המאגר המפורט ו-Decisions Log. קרא את הסעיף הרלוונטי למשימה, לא את הקובץ כולו.
   קובע לגבי **היסטוריית החלטות ופירוט טכני עמוק**. כל החלטה חדשה נרשמת כאן.
3. **`DESIGN.md`** — מקור האמת לעיצוב. גובר על קבצי `ref/` בקונפליקט, למעט חריגים מוצהרים.
4. **`TESTING.md`** — הרצת בדיקות. **`GIT_PUSH_SKILL.md`** — נוהל דחיפה.

**בקונפליקט בין BRIEF ל-PROJECT_CONTEXT:** על מצב נוכחי — ה-BRIEF גובר. על החלטות ופרטים טכניים — PROJECT_CONTEXT גובר. בכל מקרה דווח על הפער בסוף הסשן.

**אם `ALMOG_BILLING_BRIEF.md` חסר מהשורש** — אל תמשיך ואל תנחש. דווח מיד שהוא חסר — רונן מעלה אותו ב-scp.
