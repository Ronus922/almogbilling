...
function ReceiptBar({ status }: { status: ReceiptStatus }) {
  ...
  return (
    <div className="flex items-center justify-center gap-4 border-t border-slate-100 bg-white py-1.5 text-[11.5px] font-medium">
      <span className="inline-flex items-center gap-1 text-slate-400">
        <Check className="h-4 w-4" /> נשלח
      </span>
      <span className={cn('inline-flex items-center gap-1', delivered ? 'text-slate-500' : 'text-slate-300')}>
        <CheckCheck className="h-4 w-4" /> נמסר
      </span>
      <span className={cn('inline-flex items-center gap-1', read ? 'font-bold text-brand' : 'text-slate-300')}>
        <CheckCheck className="h-4 w-4" strokeWidth={read ? 3 : 2} /> נקרא
      </span>
    </div>
  );
}
...
function Composer({
  ...
  return (
    <div className="border-t border-slate-200 bg-white p-3">
      {/* Ref layout: input fills the right; the controls (send / attach / emoji)
          are grouped on the left, send being the prominent blue action. */}
      <div className="flex items-end gap-2">
        <Textarea
          ...
          className={cn(
            'max-h-32 min-h-11 flex-1 resize-none rounded-2xl bg-surface-2 px-4 py-2.5',
            tooLong && 'border-red-400 bg-red-50 focus-visible:ring-red-200',
          )}
        />
        ...
        <button
          ...
          aria-label="אימוג׳י"
          title="אימוג׳י"
          className="grid h-10 w-10 shrink-0 place-items-center rounded-full text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600"
        >
        ...
        {/* Send — prominent blue action (matches the ref). */}
        <button
          type="button"
          onClick={() => void send()}
          disabled={!canSend}
