...
/**
 * Storage for WhatsApp BROADCAST attachments (wa_campaign_attachments). All
 * Storage access goes through ./server.ts.
 *
 * The bucket is PRIVATE: the browser reaches a file only through
 * /api/files/whatsapp-attachments/<key> (session + whatsapp_chat:view), and the
...
/** Uploads a staged broadcast attachment. `mimeType` is the canonical MIME for
...
export async function removeBroadcastAttachment(objectKey: string): Promise<void> {
  await deleteObjects(BUCKET, [objectKey]);
}

/** In-app, permission-checked URL for a stored attachment (relative). */
export function broadcastAttachmentUrl(objectKey: string): string {
