...
import type { VendorCategoryWithCount } from '@/lib/types/vendors';
import { useEscapeKey } from '@/lib/hooks/useEscapeKey';

export function VendorCategoriesSheet({
  open,
  canEdit,
  onOpenChange,
  onChanged,
  ...
  const [rows, setRows] = useState<VendorCategoryWithCount[]>([]);
  ...
  const [deleteTarget, setDeleteTarget] = useState<VendorCategoryWithCount | null>(null);

  useEscapeKey(open && deleteTarget === null && editingId === null, () => onOpenChange(false));

  async function refetch() {
    try {
      const res = await fetch('/api/vendors/categories?scope=all', { credentials: 'include' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      setRows((await res.json()) as VendorCategoryWithCount[]);
    } catch (err) {
  ...
  async function toggleActive(cat: VendorCategoryWithCount, next: boolean) {
    setBusyId(cat.id);
    const prev = rows;
    setRows((rs) => rs.map((r) => (r.id === cat.id ? { ...r, is_active: next } : r)));
    try {
      const res = await fetch(`/api/vendors/categories/${cat.id}`, {
        method: 'PATCH',
        headers: { 'content-type': 'application/json' },
        credentials: 'include',
  ...
  async function confirmDelete() {
    ...
    try {
      const res = await fetch(`/api/vendors/categories/${deleteTarget.id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      const body = await res.json().catch(() => ({}));
