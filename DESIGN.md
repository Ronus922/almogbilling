...
מטריצת נמען×ערוץ בוטלה. במקומה **שני כרטיסי ערוץ גלובליים** — וואטסאפ / מייל — הגלויים **תמיד** בכרטיס „התראה ותזכורות” (בטופס התקלה) / „שליחת התראה” (בטופס המשימה), **בלי תלות בתזכורות**. הערוץ הנבחר נשלח **לגורם המטפל שנבחר בלבד** (הרחבה דרך `channelsToSelection` ב-submit → אותו `body.notify` שה-routes כבר צורכים; ה-routes לא משתנים). בחירת ערוץ **בלי תזכורת** = שליחה מיידית לגורם המטפל — מסלול עצמאי משל עצמו.
...
- **ולידציית ערוץ**: ערוץ **disabled + רמז ענבר** כשלאף נמען נבחר אין את הפרט (מייל / `users.notification_phone` / ספק `email` / `mobile‖phone`). ערוץ שנבחר ואז נפסל — נמחק אוטומטית (`useEffect`). השרת בודק שוב וממילא מדלג בשקט על פרט חסר.
- **שורת סיכום** בתחתית: ירוקה (`emerald-50`) „יישלח לגורם המטפל: …” כשיש ערוץ + גורם; אחרת ניטרלית „לא נבחר כלום — לא תישלח התראה מיידית”.

### 33.2 „שלח גם אליי” (self) — תזכורת-סקופד

הטוגל השלישי הוא **אופציית תזכורת**, לא שליחה מיידית. הוא **enabled רק כשקיימת תזכורת עם מועד** (`selfEnabled=hasDatedReminder`; רמז „דורש תזכורת עם מועד” כשלא). כשמסומן, התזכורת(ות) מגיעות גם למשתמש הנוכחי דרך `reminders.notify_owner` (מיגרציה **065**, boolean additive). בנוסף המשתמש מקבל **פעמון in-app מיידי** („נקבעה תזכורת שתגיע גם אליך”, `dedupeKey: reminder_self:<id>:<user>`). אין מייל/וואטסאפ מיידי לעצמי.
