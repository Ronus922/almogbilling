...
| בורר סוג פר-מספר (`.typemini` בשורת ההוספה) + badge סוג על כל תגית (אייקון+טקסט בגבול inline-start) | Chip2 ‏1:1; התווית "פיזי" (לא "כרטיס" של Chip2) — הנחיה גוברת, אחידות אוצר-מילים |
| בורר תפקידים בתוך בלוק | **הנחיה גוברת על Chip2**: כרטיסי ה-2×2 העשירים נשארים (Chip2 הציג שורה קומפקטית); מצב "תפוס" (`נבחר בבלוק אחר`) מאומץ מעיצוב `.taken` של Chip2 (רקע alt, ✓ רפאים, not-allowed) |
...
| Toggle על צ׳יפ שמור בתוך החלון | אקסטרפולציה + חוק מוצר: פותח את דיאלוגי ההשבתה/החזרה הקיימים **מיידית** — אין toggle שקט ואין מחיקה |
