...
**העיצוב נגזר מ-`ref/proof/whatsapp-broadcast/Chip.html` + `Chip.md` — הרפרנס גובר על מסמך זה בכל התנגשות, בהיקף מודול הצ׳יפים בלבד** (החלטת מוצר). ה-scope נאכף טכנית: כל הטוקנים מוגדרים כ-CSS vars תחת המחלקה **`.chips-skin`** (`src/app/styles/chips.css`) — שום ערך לא דורס את ה-`@theme` הגלובלי, ו-`font-num` הגלובלי נשאר Inter. אסור לייבא את פלטת הצ׳יפים למסכים אחרים.
