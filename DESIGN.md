...
| Toggle על צ׳יפ שמור בתוך החלון | **הנחיה גוברת (09/2026 — מחליפה את חוק הדיאלוגים):** פעולה מיידית בלחיצה אחת, **אפס דיאלוגים**. כיבוי → `window.confirm("להשבית את צ׳יפ <מספר>?")` בלבד, ואז `POST …/deactivate` עם `reason='unknown'` ו-`controller_synced=false`; הדלקה → `POST …/reactivate` בלי סיבה ובלי שאלה. optimistic: התגית מתחלפת מיד, כשל (כולל 409 "מספר תפוס") מחזיר אותה + `toast.error`. סדר התגית: **[טוגל] [badge סטטוס] [מספר] [badge סוג]**. אין מחיקה — צ׳יפ שהונפק לעולם לא נמחק |
